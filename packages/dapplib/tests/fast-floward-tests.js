...
describe('Flow Dapp Tests', async () => {
	...
	describe('Fast Floward Tests', async () => {
		it(`sets up two accounts`, async () => {
			...
			await DappLib.kittyItemsMarketSetupAccount(
				testData1
			);
			await DappLib.kittyItemsMarketSetupAccount(
				testData2
		...
		it(`cannot mint 0 kibble and doesn't affect supply`, async () => {
			...
			try {
				await DappLib.kibbleMintTokens(
					testData1
				);
			} catch (e) {
		...
		it(`cannot withdraw more than vault has but balances and supply remain the same`, async () => {
			...
			try {
				await DappLib.kibbleTransferTokens(
					testData1
				);
			} catch (e) {
				let res1 =
					await DappLib.kibbleGetBalance(
						testData2
					);
				let res2 =
					await DappLib.kibbleGetBalance(
		...
		it(`has 0 initial kittyitems in the supply`, async () => {
			let res =
				await DappLib.kittyItemsReadKittyItemsSupply(
					{}
				);
			...
			assert.equal(
				res.result,
				0,
				'There should be 0 initial KittyItems'
			);
		});
		...
		it(`mints 2 kittyitems into admin account and has correct collection information`, async () => {
			...
			let res1 =
				await DappLib.kittyItemsReadCollectionIDs(
					testData2
				);
			let res2 =
				await DappLib.kittyItemsReadCollectionLength(
					testData2
				);
			...
			assert.equal(
				res1.result[0],
				0,
				'Incorrect ID in KittyItems collection'
			);
			assert.equal(
				res1.result[1],
				1,
			...
			assert.equal(
				res2.result,
				2,
				'Incorrect length of KittyItems collection'
			);
		});
		...
		it(`has correct total number of kittyitems and kittyitem typeID is correct`, async () => {
			...
			let res2 =
				await DappLib.kittyItemsReadKittyItemTypeID(
					testData
				);
			...
			assert.equal(
				res1.result,
				2,
				'Incorrect total number of KittyItems'
			);

			assert.equal(
				res2.result,
				5,
				'KittyItem has incorrect typeID'
			);
		});
		...
		it(`safely fails when withdrawing kittyitem with wrong id`, async () => {
			...
			try {
				await DappLib.kittyItemsTransferKittyItem(
					testData1
				);
			} catch (e) {
				let res1 =
		...
		it(`transfers 2 kittyitems from admin to user and has correct collection information`, async () => {
			...
			assert.equal(
				res1.result,
				0,
				'Admin should have no KittyItems left'
			);
			assert.equal(
				res2.result,
				2,
				'User should have a KittyItem'
			);
			assert.equal(
				res3.result[0],
				0,
		...
		it(`lists 2 kittyitems for sale`, async () => {
			...
			await DappLib.kittyItemsMarketSellMarketItem(
				testData1
			);
			await DappLib.kittyItemsMarketSellMarketItem(
				testData2
			...
			let res1 =
				await DappLib.kittyItemsMarketReadSaleCollectionIDs(
					testData3
				);
			let res2 =
				await DappLib.kittyItemsMarketReadSaleCollectionLength(
					testData4
				);
			...
			assert.equal(
				res2.result,
				2,
				'User should have 2 KittyItems up for sale'
			);
		});
		...
		it(`removes a kittyitem from sale`, async () => {
			...
			await DappLib.kittyItemsMarketRemoveMarketItem(
				testData1
			);
		...
		it(`buys a kittyitem`, async () => {
			...
			try {
				await DappLib.kittyItemsMarketBuyMarketItem(
					testData1
				);
			} catch (e) {
				console.log(e);
		...
		it(`checks admin actually has a kittyitem in collection`, async () => {
			...
			assert.equal(
				res1.result[0],
				1,
				'Incorrect ID in KittyItem collection'
			);
			assert.equal(
				res2.result,
				1,
				'Incorrect length of KittyItem collection'
			);
		});
		...
		it(`checks both admin and user have correct balances after purchase`, async () => {
			...
			let bal0 = await DappLib.kibbleGetBalance(
				testData1
			);
			// 4) call kibbleGetBalance with testData2 and store the result
			let bal1 = await DappLib.kibbleGetBalance(
				testData2
			);
