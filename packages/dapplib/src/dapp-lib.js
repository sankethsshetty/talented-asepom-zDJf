				...
				if (error) {
					...
					callback({
						...
						label:
							'Event ' +
							event,
						result: DappLib.getObjectNamedProperties(
							result
						),
					});
				}
	...
	static getFormattedResultNode(retVal, key) {
		...
		switch (retVal.type) {
			...
			case DappLib.DAPP_RESULT_TX_HASH:
				formatted = DappLib.formatTxHash(
					retVal[returnKey]
				);
				break;
			case DappLib.DAPP_RESULT_ACCOUNT:
				formatted = DappLib.formatAccount(
					retVal[returnKey]
				);
				break;
			case DappLib.DAPP_RESULT_BOOLEAN:
				formatted = DappLib.formatBoolean(
					retVal[returnKey]
				);
				break;
			...
			case DappLib.DAPP_RESULT_OBJECT:
				formatted = DappLib.formatObject(
					retVal[returnKey]
				);
				break;
			default:
		...
		resultNode.innerHTML =
			...
			`${
				retVal.type ===
				DappLib.DAPP_RESULT_ERROR
					? '☹️'
					: '👍️'
			} ` +
			(Array.isArray(retVal[returnKey])
				? 'Result'
				: retVal.label) +
			': ' +
			formatted +
